...
export default function EditarPerfil({
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (!loggedInUserEmail) {
        setErrorMessage("Email do usuário não disponível para buscar dados.");
        return;
      }
      setLoading(true);
      ...
      try {
        ...
        if (response.ok) {
          ...
          setCurrentProfilePhoto(
            data.profilePhotoUrl || "/path/to/default-avatar.png"
          );
        } else {
        ...
        console.error(
          "Erro ao comunicar com o backend para carregar dados do usuário:",
          error
        );
        setErrorMessage("Erro de conexão ao carregar dados do usuário.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setNewProfilePhotoPreview(reader.result);
      };
  ...
  const handlePhotoUpload = async () => {
    if (!selectedFile) {
      setErrorMessage("Nenhuma nova foto selecionada.");
      return;
    }
    setLoading(true);
    setErrorMessage(null);
    ...
    try {
      const response = await fetch(`${backendUrl}/users/update-profile-photo`, {
        method: "PUT",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        setCurrentProfilePhoto(newProfilePhotoPreview);
        setNewProfilePhotoPreview(null);
        setSelectedFile(null);
        if (onUserUpdate)
          onUserUpdate({ ...userData, profilePhotoUrl: data.newPhotoUrl });
      } else {
        const errorData = await response.json();
        setErrorMessage(
          `Erro ao atualizar foto: ${errorData.message || "Erro desconhecido"}`
        );
      }
    } catch (error) {
      console.error("Erro ao enviar foto de perfil:", error);
      setErrorMessage("Erro ao comunicar com o backend para atualizar a foto.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateEmail = async (event) => {
    ...
    if (!email || email === userData.email) {
      setErrorMessage("O novo email não pode ser vazio ou igual ao atual.");
      return;
    }
    setLoading(true);
    setErrorMessage(null);
    ...
    try {
      const response = await fetch(`${backendUrl}/users/update-email`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ oldEmail: userData.email, newEmail: email }),
      });

      if (response.ok) {
        setSuccessMessage("Email atualizado com sucesso!");
        setUserData((prev) => ({ ...prev, email: email }));
        if (onUserUpdate) onUserUpdate({ ...userData, email: email });
      } else {
        const errorData = await response.json();
        setErrorMessage(
          `Erro ao atualizar email: ${errorData.message || "Erro desconhecido"}`
        );
      }
    } catch (error) {
      console.error("Erro ao atualizar email:", error);
      setErrorMessage(
        "Erro ao comunicar com o backend para atualizar o email."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleChangePassword = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setErrorMessage(
          `Erro ao alterar senha: ${errorData.message || "Erro desconhecido"}`
        );
      }
    } catch (error) {
      console.error("Erro ao alterar senha:", error);
      setErrorMessage("Erro ao comunicar com o backend para alterar a senha.");
    } finally {
      setLoading(false);
    }
  };
      ...
      onConfirm: async () => {
        ...
        try {
          const response = await fetch(
            `${backendUrl}/users/delete-account?email=${userData.email}`,
            {
              method: "DELETE",
            }
          );
          ...
          setErrorMessage(
            "Erro ao comunicar com o backend para deletar a conta."
          );
        } finally {
          setLoading(false);
        }
